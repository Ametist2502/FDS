...
class Game:
    ...
    def spawn_insects(self):
        ...
        if t > self.insects_spawn_timer:
            ...
            nb = (GAME_DURATION-self.time_left)/GAME_DURATION * 80   # increase from 0 to 50 during all  the game (linear)
            if random.randint(0, 120) < nb:
    ...
    def update(self):
        ...
        if self.time_left > 0:
            ...
            if self.hand.stun > 0:
                self.hand.image = self.hand.stun_image.copy()
            elif self.hand.left_click:
                self.hand.image = self.hand.image_smaller.copy()
            else:
                self.hand.image = self.hand.orig_image.copy()
            self.score, self.mosquito, self.bee = \
                self.hand.kill_insects(self.insects, self.score, self.mosquito, self.bee, self.sounds)
            for insect in self.insects:
                insect.move()
